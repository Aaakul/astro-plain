  ...
  .use(function fileTree() {
    return (tree: Element, file) => {
      ...
      visit(tree, "element", (node) => {
        ...
        comment.push(...(commentNodes as unknown as Child[]));

        const firstChildTextContent = firstChild
          ? toString(firstChild as unknown as Parameters<typeof toString>[0])
          : "";
        ...
        const treeEntryChildren: Child[] = [
          h(
            ...
            ...(isPlaceholder ? [] : [icon]),
            ...(firstChild ? [firstChild as unknown as Child] : []),
          ),
        ];
        ...
        if (isDirectory) {
          ...
          node.children = [
            h(
              ...
              { open: hasContents },
              h("summary", treeEntry),
              ...(hasContents ? (otherChildren as unknown as Child[]) : [h("ul", h("li", "…"))]),
            ),
        ...
        node.children = [
          treeEntry,
          ...(otherChildren as unknown as Child[]),
        ] as unknown as typeof node.children;
...
function validateFileTree(tree: Element) {
  ...
  const listItemElement = select("li", rootElement as unknown as Parameters<typeof select>[1]);
